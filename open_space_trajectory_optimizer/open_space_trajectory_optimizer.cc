...
int OpenSpaceTrajectoryOptimizer::Plan(
    ...
    */
    ...
    y0 y1...yn
    */
    Eigen::MatrixXd f_driving_bound =
        Eigen::MatrixXd::Zero(4, num_point);  // 4*n
    Eigen::MatrixXd b_driving_bound =
        Eigen::MatrixXd::Zero(4, num_point);  // 4*n
...
bool OpenSpaceTrajectoryOptimizer::GenerateDistanceApproachTraj(
    ...
    if (distance_approach_->Solve(x0, xF, last_time_u, horizon, ts_init, ego,
                                  xWS, uWS, XYbounds, obstacles_num,
                                  obstacles_edges_num, f_driving_bound,
                                  b_driving_bound, state_result_ds,
                                  control_result_ds, time_result_ds)) {
...
void OpenSpaceTrajectoryOptimizer::LoadCoarseTrajectory(
    ...
    for (size_t i = 0; i < states_size; ++i) {
        ...
        if (i == 0) {
            ...
            relative_time =
                config_.planner_open_space_config().warm_start_config.delta_t *
                i;
            point.set_relative_time(relative_time);
        }

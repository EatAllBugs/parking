...
bool ApolloOpenSpaceTrajectoryOptimizer::Plan(
    ...
    if (FLAGS_enable_parallel_trajectory_smoothing) {
        ...
        if (!warm_start_->TrajectoryPartition(result,
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            //     AINFO << "trajectory partitioned size: " <<
            //     xWS_vec[i].cols(); AINFO << "initial point: " <<
            //     xWS_vec[i].col(0).transpose(); AINFO << "ending point: "
            ...
            if (FLAGS_use_iterative_anchoring_smoother) {
                if (!GenerateDecoupledTraj(
                    ...
                    std::cout << i << "th trajectory size is "
                              << xWS_vec[i].cols();
                    return 0;
                }
            } else {
                ...
                std::cout << "total planning time spend: " << time_diff_ms
                          << " ms.";
                std::cout << i << "th trajectory size is " << xWS_vec[i].cols();
                std::cout << "average time spend: "
                          << time_diff_ms / xWS_vec[i].cols()
                          << " ms per point.";
...
bool ApolloOpenSpaceTrajectoryOptimizer::GenerateDecoupledTraj(
    ...
    //         xWS, init_a, init_v, obstacles_vertices_vec,
    //         &smoothed_trajectory))
    //         {
    //   return false;
    // }

...
DistanceApproachIPOPTCorridorInterface::DistanceApproachIPOPTCorridorInterface(
    ...
    : ds_init_(ds_init),  // ts采样时间间隔
      ...
      f_driving_bound_(f_driving_bound),
      b_driving_bound_(b_driving_bound) {
    ...
    // dual_l_result_ = Eigen::MatrixXd::Zero(obstacles_edges_sum_, horizon_ +
    // 1); dual_n_result_ = Eigen::MatrixXd::Zero(4 * obstacles_num_, horizon_ +
    // 1);
...
bool DistanceApproachIPOPTCorridorInterface::get_bounds_info(
...
bool DistanceApproachIPOPTCorridorInterface::eval_f(int n,
                                                    const double *x,
                                                    bool new_x,
                                                    double &obj_value) {
...
bool DistanceApproachIPOPTCorridorInterface::eval_grad_f(int n,
                                                         const double *x,
                                                         bool new_x,
                                                         double *grad_f) {
...
bool DistanceApproachIPOPTCorridorInterface::eval_g(
...
bool DistanceApproachIPOPTCorridorInterface::eval_jac_g(int n,
                                                        const double *x,
                                                        bool new_x,
                                                        int m,
                                                        int nele_jac,
...
bool DistanceApproachIPOPTCorridorInterface::eval_jac_g_ser(int n,
                                                            const double *x,
                                                            bool new_x,
                                                            int m,
                                                            int nele_jac,
    ...
    if (values == nullptr) {
        ...
        std::cout << "After fulfilled control rate constraints derivative, "
                     "nz_index : "
...
bool DistanceApproachIPOPTCorridorInterface::eval_h(int n,
                                                    const double *x,
                                                    bool new_x,
                                                    double obj_factor,
                                                    int m,
...
//***************    start ADOL-C part ***********************************
template <class T>
void DistanceApproachIPOPTCorridorInterface::eval_obj(int n,
                                                      const T *x,
                                                      T *obj_value) {
    // Objective is :
    ...
    *obj_value = 0.0;
    ...
    for (int i = 0; i < horizon_ - 1; ++i) {
        T steering_rate =
            (x[control_index + 2] - x[control_index]) / x[time_index];
        T a_rate =
            (x[control_index + 3] - x[control_index + 1]) / x[time_index];
...
template <class T>
void DistanceApproachIPOPTCorridorInterface::eval_constraints(int n,
                                                              const T *x,
                                                              int m,
                                                              T *g) {
    ...
    for (int i = 0; i < horizon_; ++i) {
        ...
        g[constraint_index] =
            x[state_index + 4] -
            (x[state_index] +
             x[time_index] * x[state_index + 3] * cos(x[state_index + 2]));
    ...
    g[constraint_index] =
        (x[control_index] - last_time_u_(0, 0)) / x[time_index];
    control_index += 2;
    constraint_index++;
    ...
    for (int i = 0; i < horizon_ + 1; i++) {
        ...
        // vehicle_config_.back_edge_to_center) * cos(theta); double y_f = y +
        // (3 / 4 * vehicle_config_.length -
        // vehicle_config_.back_edge_to_center) * sin(theta); double x_b = x +
        // (1 / 4 * vehicle_config_.length -
        // vehicle_config_.back_edge_to_center) * cos(theta); double y_b = y +
        // (1 / 4 * vehicle_config_.length -
        // vehicle_config_.back_edge_to_center) * sin(theta); x_f
        g[constraint_index] =
            x[state_index] + (3 / 4 * vehicle_param_.length -
                              vehicle_param_.back_edge_to_center) *
                                 cos(x[state_index + 2]);
        // y_f
        g[constraint_index + 1] =
            x[state_index + 1] + (3 / 4 * vehicle_param_.length -
                                  vehicle_param_.back_edge_to_center) *
                                     sin(x[state_index + 2]);
        // x_b
        g[constraint_index + 2] =
            x[state_index] + (1 / 4 * vehicle_param_.length -
                              vehicle_param_.back_edge_to_center) *
                                 cos(x[state_index + 2]);
        // y_b
        g[constraint_index + 3] =
            x[state_index + 1] + (1 / 4 * vehicle_param_.length -
                                  vehicle_param_.back_edge_to_center) *
                                     sin(x[state_index + 2]);
        state_index += 4;
        constraint_index += 4;
...
bool DistanceApproachIPOPTCorridorInterface::check_g(int n,
                                                     const double *x,
                                                     int m,
                                                     const double *g) {
    int kN = n;
    ...
    for (int idx = 0; idx < m; ++idx) {
        if (g[idx] > g_u_tmp[idx] + delta_v ||
            ...
            std::cout << "constrains idx unfeasible: " << idx
                      << ", g: " << g[idx] << ", lower: " << g_l_tmp[idx]
                      << ", upper: " << g_u_tmp[idx] << std::endl;
        }
    }
    return true;
}
// IPOPT APP step 2
void DistanceApproachIPOPTCorridorInterface::generate_tapes(int n,
                                                            int m,
                                                            int *nnz_jac_g,
                                                            int *nnz_h_lag) {

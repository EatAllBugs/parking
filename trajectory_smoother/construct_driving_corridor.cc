...
ConstructDrivingCorridor::ConstructDrivingCorridor(
    ...
    : open_space_config_(open_space_config),
      vehicle_config_(vehicle_config) {}  //应该进行相关的初始化工作
...
int ConstructDrivingCorridor::Construct(
    ...
    double swelling_r = pow(
        (vehicle_config_.width / 2.0) * (vehicle_config_.width / 4.0) +
            (vehicle_config_.length / 4.0) * (vehicle_config_.length / 4.0) -
            1.5,
        0.5);
...
void ConstructDrivingCorridor::SwellingObstacles(
    ...
    const double &r,
    std::vector<common::math::Vec2d> *swelling_obstacles_vertices) {
    std::vector<common::math::Vec2d> temp_obstacles_vertices =
        obstacles_vertices;
    ...
    temp_obstacles_vertices.insert(temp_obstacles_vertices.begin(),
                                   last_vertice);
...
int ConstructDrivingCorridor::CollisionCheck(
    ...
    bool *result) {
    if (driving_box_vertices.size() != 4 ||
        convex_polygon_vertices.size() < 3) {
        std::cout << "输入错误:可行框不是4个顶点,障碍物少于3个顶点"
                  << std::endl;
        return 0;
    } else {
        ...
        } else  //如果AABB碰撞，则进行精确碰撞检测
        {
            ...
            for (int i = 0; i < convex_polygon_vertices.size(); i++) {
                ...
                if (i == 0)
                    vec_edge = convex_polygon_vertices[0] -
                               convex_polygon_vertices.back();
                else
                    vec_edge = convex_polygon_vertices[i] -
                               convex_polygon_vertices[i - 1];
            ...
            for (const auto &project_axis : project_axis_vec) {
                ...
                double box_x_max = -DBL_MAX, box_x_min = DBL_MAX,
                       obs_x_max = -DBL_MAX, obs_x_min = DBL_MAX;

                for (
                    const auto &driving_box_vertice :
                    driving_box_vertices)  //将可行驶框投影到投影轴，并按照x排序
                {
                    double project =
                        project_axis.InnerProd(driving_box_vertice);
                    box_projects.push_back(project);
                    if (project > box_x_max) box_x_max = project;
                    if (project < box_x_min) box_x_min = project;
                }
                ...
                {
                    double project =
                        project_axis.InnerProd(convex_polygon_vertice);
                    obs_projects.push_back(project);
                    if (project > obs_x_max) obs_x_max = project;
                    if (project < obs_x_min) obs_x_min = project;
                }

            ...
            for (const common::math::LineSegment2d &linesegment :
                ...
                {  // 碰撞，包围框是否覆盖障碍物的边
                    if (bounding_box.HasOverlap(linesegment)) {
                        std::cout << "collision start at x,y: "
                                  << linesegment.start().x() << " , "
                                  << linesegment.start().y() << std::endl;
                        std::cout << "collision end at   x,y: "
                                  << linesegment.end().x() << " , "
...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(
    ...
    // 其次，车辆在当前的姿态下，既可以向左转、又可以向右转，那么steering
    // angle的 取值范围其实是[-max_steer_angle_,
    // max_steer_angle_]，在正向或反向下， 能取next_node_num_/2个有效值。
    // 即，把[-max_steer_angle_, max_steer_angle_]分为（next_node_num_/2-1）份
    // 所以，steering = 初始偏移量 + 单位间隔 × index

    if (next_node_index <
        static_cast<double>(next_node_num_) / 2) {  // 正向，前5
...
bool HybridAStar::TrajectoryPartition(
    ...
    std::cout << "speed profile total time: " << diff.count() * 1000.0
              << " ms.";
    return true;
}
...
bool HybridAStar::Plan(double sx,
    ...
    for (const auto &obstacle_vertices : obstacles_vertices_vec) {
        ...
        common::math::LineSegment2d last_line_segment =
            common::math::LineSegment2d(obstacle_vertices[vertices_num - 1],
                                        obstacle_vertices[0]);
    ...
    while (!open_pq_.empty()) {
        ...
        const std::string current_id =
            open_pq_.top().first;  // 取代价最小的相邻节点
        open_pq_.pop();            // 从队列中取出最小值
        std::shared_ptr<Node3d> current_node =
            open_set_[current_id];  // 存储当前节点
        ...
        // search ends.如果有一条RS曲线连接当前点和终点无碰撞，那么终止搜索
        ...
        for (size_t i = 0; i < next_node_num_; ++i) {  // 10个方向扩展，上5下5
            ...
            // check if the node is already in the close
            // set检测是否已经在close_set

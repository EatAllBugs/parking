...
bool ReedShepp::ShortestRSP(
    const std::shared_ptr<Node3d> start_node,  // 起点节点
    ...
    if (!GenerateRSPs(start_node, end_node, &all_possible_paths)) {
        std::cout
            << "Fail to generate different combination of Reed Shepp paths"
            << std::endl;
        return false;
    }
    // 找到路径最小值
    ...
    if (std::abs(all_possible_paths[optimal_path_index].x.back() -
        ...
        for (size_t i = 0;
             i < all_possible_paths[optimal_path_index].segs_types.size();
             ++i) {
            std::cout << "types are "
...
void ReedShepp::LSL(const double x,
                    const double y,
                    const double phi,
                    RSPParam *param) {
...
void ReedShepp::LSR(const double x,
                    const double y,
                    const double phi,
                    RSPParam *param) {
    std::pair<double, double> polar = common::math::Cartesian2Polar(
...
void ReedShepp::LRL(const double x,
                    const double y,
                    const double phi,
                    RSPParam *param) {
    std::pair<double, double> polar = common::math::Cartesian2Polar(
...
void ReedShepp::SLS(const double x,
                    const double y,
                    const double phi,
                    RSPParam *param) {
...
void ReedShepp::LRLRn(const double x,
                      const double y,
                      const double phi,
                      RSPParam *param) {
...
void ReedShepp::LRLRp(const double x,
                      const double y,
                      const double phi,
                      RSPParam *param) {
    double xi = x + std::sin(phi);
...
void ReedShepp::LRSR(const double x,
                     const double y,
                     const double phi,
                     RSPParam *param) {
    double xi = x + std::sin(phi);
...
void ReedShepp::LRSL(const double x,
                     const double y,
                     const double phi,
                     RSPParam *param) {
    double xi = x - std::sin(phi);
...
void ReedShepp::LRSLR(const double x,
                      const double y,
                      const double phi,
                      RSPParam *param) {
    double xi = x + std::sin(phi);

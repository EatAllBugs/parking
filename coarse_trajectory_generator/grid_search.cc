...
bool GridSearch::GenerateAStarPath(
    ...
    XYbounds_ = XYbounds;
    std::shared_ptr<Node2d> start_node = std::make_shared<Node2d>(
        sx, sy, xy_grid_resolution_, XYbounds_);  // 起点
    std::shared_ptr<Node2d> end_node = std::make_shared<Node2d>(
        ex, ey, xy_grid_resolution_, XYbounds_);  // 终点
...
bool GridSearch::GenerateDpMap(
    ...
    open_pq.emplace(end_node->GetIndex(),
                    end_node->GetCost());  // 终点放入open_pq
    ...
    while (!open_pq.empty()) {
        ...
        for (auto &next_node : next_nodes) {
            ...
            if (open_set.find(next_node->GetIndex()) ==
                ...
                open_set.emplace(next_node->GetIndex(),
                                 next_node);  // 加入open_set
                open_pq.emplace(next_node->GetIndex(),
                                next_node->GetCost());  // 加入pq
